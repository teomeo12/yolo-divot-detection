...
def initialize_camera():
    ...
    print(f"fx: {fx}, fy: {fy}")
    print("Pipeline started. Press 'q' to quit, 'SPACE' to start/stop recording.")
...
def create_video_writer(width, height, fps=20):
    ...
    # Create YOLO11s_videos directory if it doesn't exist
    video_dir = "YOLO11s_videos"
    if not os.path.exists(video_dir):
        os.makedirs(video_dir)
    
    # Generate unique filename with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"divot_analysis_{timestamp}.mp4"
