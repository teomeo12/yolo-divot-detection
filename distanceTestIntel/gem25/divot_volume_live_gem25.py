...
def calculate_volume_and_depth(divot_contour, depth_image, fx, fy):
    ...
    divot_mask_binary = np.zeros(depth_image.shape[:2], dtype="uint8")
...
def main():
    # Initialize YOLO model - Correct path to the model in yolo11s_1600_100ep folder
    model_path = r"../../yolo11s_1600_100ep/1600s_aug_100ep.pt"
        ...
        while True:
            ...
            if len(detections) > 0:
                ...
                if detections.mask is not None and len(detections.mask) > detection_idx:
                    ...
                    divot_contour, _ = convert_yolo_mask_to_contour(detections.mask[detection_idx])
                    
                    if divot_contour is not None:
                        # Calculate volume and depth
                        measurements = calculate_volume_and_depth(divot_contour, depth_image, fx, fy)
                        ...
                        if measurements and 'error' not in measurements:
                            ...
                            cv2.putText(display_image, measurements.get('error', 'Calc Error'), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            
            # Add YOLO annotations for context (optional)
